...
export function BookingModal({ isOpen, onClose, professional }: BookingModalProps) {
  ...
  const services = professional.services || [
    { id: "default", title: "Initial Consultation", price: professional.consultationFee, duration: 30 },
  ];
        ...
        <CardContent className="p-6 space-y-4">
          <div className="flex items-center gap-4 pb-4 border-b border-slate-200">
            <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-600 to-teal-500 flex items-center justify-center text-white font-bold text-lg">
              {professional.user?.name?.split(" ").map(n => n[0]).join("").slice(0, 2)}
            </div>
            <div>
          ...
          <div className="space-y-3">
            ...
            <div className="pt-3 border-t border-slate-200">
              <div className="flex justify-between">
                ...
                <span className="text-xl font-bold text-blue-600">{formatCurrency(selectedService?.price || 0)}</span>
              </div>
            </div>
          </div>
        </CardContent>
  ...
  const renderSuccessStep = () => (
    <div className="text-center py-8">
      ...
      <p className="text-slate-500 mb-6">
        Your appointment with {professional.user?.name} has been scheduled for {selectedDate && formatDate(selectedDate)} at {selectedTime}
      </p>
  ...
  const getStepTitle = () => {
    switch (step) {
      case "service": return "Book Appointment";
            ...
            {["service", "datetime", "confirm"].map((s, i) => (
              <React.Fragment key={s}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium transition-colors ${
                  ...
                  ["service", "datetime", "confirm"].indexOf(step) > i ? "bg-emerald-500 text-white" : "bg-slate-200 text-slate-500"
                }`}>
                  {["service", "datetime", "confirm"].indexOf(step) > i ? (
                    <CheckCircle className="w-5 h-5" />
                  ) : i + 1}
                </div>
                {i < 2 && <div className={`w-12 h-1 rounded ${["service", "datetime", "confirm"].indexOf(step) > i ? "bg-emerald-500" : "bg-slate-200"}`} />}
              </React.Fragment>
            ))}
          </div>
        )}
